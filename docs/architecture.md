...
**Runtime validation** — complete under admin (2026-04-28, re-confirmed 2026-05-06). Tests 1–5 all pass: Job Object containment, Low Integrity token, project root SACL, WFP outbound block, ETW audit capture. Two bugs fixed during validation: `KILL_ON_JOB_CLOSE` required `ExtendedLimitInformation` (class 9) not `BasicLimitInformation` (class 2) on Windows 11; WFP filter add required a non-null `displayData.name`. WFP violation streaming pipeline is verified; end-to-end block-triggers-event not yet exercised against a live agent initiating outbound traffic.
...
**WFP violation streaming** — pipeline complete; end-to-end pending. `WfpEventMonitor` subscribes via `FwpmNetEventSubscribe0` and surfaces blocked connections as `SessionEventRecord::Audit` entries in the active session. Events are drained and persisted at `stop_session`. The filter installs and subscribes correctly under admin. End-to-end validation (block triggers a JSONL event) is not yet confirmed: test instrumentation used a `.cmd` shim while `resolve_app_path` resolves via SearchPathW with a hardcoded `.exe` extension, causing the filter image and spawned process image to diverge. If the subscription fails at Medium integrity (access-denied), a descriptive hint is emitted to stderr.
